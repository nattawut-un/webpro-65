  ...
  <SectionFull title="ตะกร้าสินค้า" :image="CartImage" backButton="true">
    ...
    <div class="flex my-4" v-for="item in store.cart">
      <div class="w-1/6 mr-8">
        <img :src="item.images[0].path" class="w-full aspect-square rounded-full object-cover shadow-lg">
      </div>
      <div class="w-3/4 flex">
        ...
        <div class="w-1/3 text-end">
          ...
          <button class="bg-red-500 text-white rounded-full px-4 py-2" @click="removeFromCart(store.cart.indexOf(item))" :disabled="store.cart.length == 0">
            ลบ
          </button>
        </div>
      </div>
